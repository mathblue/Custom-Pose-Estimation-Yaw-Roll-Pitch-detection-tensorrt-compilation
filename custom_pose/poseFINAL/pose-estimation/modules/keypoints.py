...
def group_keypoints(all_keypoints_by_type, pafs, pose_entry_size=2+num_keys, min_paf_score=min_score, demo=False):
    pose_entries = []
    ...
    for part_id in range(len(BODY_PARTS_PAF_IDS)):
        ...
        elif part_id == num_keys-1 or part_id == num_keys:
            kpt_a_id = BODY_PARTS_KPT_IDS[part_id][0]
            kpt_b_id = BODY_PARTS_KPT_IDS[part_id][1]
            for i in range(len(connections)):
                for j in range(len(pose_entries)):
    ...
    if print_pose_entries:
        print(pose_entries)
    if print_allkeypoints:
        print(all_keypoints)
