...
def run_demo(net, image_provider, height_size, cpu, track_ids):
    ...
    for img in image_provider:
        ...
        if(len(pose_entries)>0):
            ...
            if allkey:
                for l in range(4):
                    landmarks.append(int(all_keypoints[int(pose_entries[0][l])][0]))
                    landmarks.append(int(all_keypoints[int(pose_entries[0][l])][1]))
        if len(landmarks)>0:
            imgpts, modelpts, rotate_degree, center = face_orientation(img, landmarks)
        ...
        cv2.imwrite('/home/charles/DockerData/Mattia/pose/poseFINAL/out.jpg', img)
