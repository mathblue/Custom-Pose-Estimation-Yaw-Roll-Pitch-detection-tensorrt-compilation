...
def face_orientation(frame, landmarks, ear):
    size = frame.shape #(height, width, color_channel)
    length=math.sqrt((landmarks[6]+landmarks[0])**2+(landmarks[7]+landmarks[1])**2)
    image_points = np.array([
                            (landmarks[0], landmarks[1]),     # nose
                            (landmarks[2], landmarks[3]),     # R eye
                            (landmarks[4], landmarks[5]),     # L eye
                            (landmarks[6], landmarks[7])      # R/L ear
                        ], dtype="double")

    

    if ear=="right":
        v=(150/4.5*10, 0.0, -150/4.5*13.5)
    else:
        v=(-150/4.5*10, 0.0, -150/4.5*13.5)
    model_points = np.array([
                            (0.0, 0.0, 0.0),             # nose
                            (150.0, 170.0, -135.0),     # R eye
                            (-150.0, 170.0, -135.0),       # L eye
                            v #R/L ear      
                        ])
